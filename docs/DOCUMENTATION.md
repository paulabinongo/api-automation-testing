...
**DRAFT retention (mock policy):** **`POST /v1/loan-applications`** sets **`draft_created_at`**. **`DELETE /v1/loan-applications/{applicationId}`** abandons a **DRAFT** only after a **minimum retention** period — **60 seconds** by default — so the system or borrower cannot cancel a fresh draft inside that window (**409**). Lower the threshold for tests with **`DRAFT_MIN_RETENTION_MS`** (e.g. **`100`**; **`npm run test:integration`** sets this on the mock and Vitest). Optional **`DRAFT_SYSTEM_CANCEL_AFTER_MS`** (positive number, ms) turns on **server-side** removal of stale **DRAFT**s after that age, but never sooner than **`DRAFT_MIN_RETENTION_MS`** / default minimum.
...
| **1**                         | Start a new application (draft).                                | `POST /v1/loan-applications`                                                                                               | Full **Metrobank-style** intake: **`metrobank_client_type`**, **`loan_purpose`**, **`additional_information`** (PEP booleans; optional **`metrobank_deposit_repayment_plan`** for **`NOT_METROBANK_CLIENT`** / **`EXISTING_CLIENT_CREDIT_CARD`**), **`borrower`**, **`employment`** (see **§5.2**). Eligibility must pass (**422** if not). Principal = **whole PHP** (centavos **÷ 100**). | Bearer + KYC done                                                        | **`application_id`** ← response **`id`**. **`loan_id` is `null`** (normal).      |
...
| **1b-DEL** _(optional)_      | Abandon **DRAFT** (borrower cancel).                             | `DELETE /v1/loan-applications/{applicationId}`                                                                               | No body. **204** removes the draft. **409** if **`status`** is not **DRAFT**, or if the draft is younger than the configured **minimum retention** (default **60** seconds — **`DRAFT_MIN_RETENTION_MS`** on the mock; see **§3.1**).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Bearer + **application_id** (**DRAFT**, same session)                    | **—** (no JSON body).                                                            |
...
| **1c-MB** _(conditional)_     | Metrobank **deposit for ADA** confirmation (**Step 7c**).        | `POST /v1/loan-applications/{applicationId}/metrobank-deposit-account/confirm`                                              | Body `{}`. When **`metrobank_client_type`** is **`NOT_METROBANK_CLIENT`** or **`EXISTING_CLIENT_CREDIT_CARD`** and **`additional_information.metrobank_deposit_repayment_plan`** is **`WILL_OPEN_METROBANK_DEPOSIT`**. **400** if **`EXISTING_CLIENT_DEPOSIT_ACCOUNT`** (not applicable). **409** if **1c** not done, plan is missing or not **`WILL_OPEN_METROBANK_DEPOSIT`**, or status is past **underwriting** queue (not **DRAFT** / **SUBMITTED** / **IN_PROCESSING** / **CREDIT_COMPLETED** / **IN_UNDERWRITING**). Re-runs **eligibility** (**422** if failed). Sets **`metrobank_deposit_account_confirmed_at`** (cleared when **PATCH** changes **`additional_information`** or **`metrobank_client_type`**).                                                                                                                                                                                                                                                                                                                                                                                                              | Bearer + **application_id** after **1c** (up to **IN_UNDERWRITING**)                     | Timestamp required before **underwriting** can **APPROVE** / **CONDITIONAL** (not before **submit**).                              |
...
| **2**                         | Send the application to processing.                             | `POST /v1/loan-applications/{applicationId}/submit`                                                                        | No body. **Personal Loan:** **1c** required (**409** if skipped). **1c-MB** is **not** required for **submit**; **underwriting** **APPROVE** / **CONDITIONAL** stay **422** until **`metrobank_deposit_account_confirmed_at`** when **`NOT_METROBANK_CLIENT`** or **`EXISTING_CLIENT_CREDIT_CARD`**. If **either** PEP boolean **`true`**, **1c-PEP** required (**409** if skipped).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | **application_id** from step 1                                           | Status **SUBMITTED**.                                                            |
...
| —    | Metrobank deposit confirm (if **NOT / CC** + will open) | `POST /loan-applications/{applicationId}/metrobank-deposit-account/confirm` — after **1c**, before **underwriting** **APPROVE**, when **`WILL_OPEN_METROBANK_DEPOSIT`** (allowed through **IN_UNDERWRITING**); not a **submit** gate (**§5.0** row **1c-MB**) |
...
| 2    | Submit                   | `POST .../submit` → **SUBMITTED** (Personal Loan: **1c** + conditional **pep-clearance**; **metrobank confirm** gates **approval**, not **submit**)                                     |
...
**Eligibility** (`javascript/lib/personalLoanEligibility.js`): **`metrobank_deposit_for_ada`** documents that a Metrobank deposit for **ADA** is required before **approval**; intake no longer fails solely on plan choice.
...
| `environments/Local_Mock.postman_environment.json`      | Local URLs, demo login fields; defaults target **no Metrobank deposit yet** (**`NOT_METROBANK_CLIENT`** + **`metrobank_deposit_repayment_plan_line`** for **`WILL_OPEN_METROBANK_DEPOSIT`**) so **Step 7c** confirm works in the happy path — see §**7.3** to switch to **already has deposit**. Placeholders for `access_token`, `application_id`, `loan_id`, `stipulation_id`, `stipulation_ids_json`                      |
...
With **Loan Lifecycle — Local mock** selected, the environment sets **`base_url`**, **`openapi_url`**, **`swagger_url`**, **`demo_login_email`**, **`demo_login_password`**, **`payment_amount_cents`**, **`payment_method`**, **`primary_id_document_type`**, and empty **`access_token`**, **`application_id`**, **`loan_id`**.

**Default borrower (no Metrobank deposit yet):** **`metrobank_client_type`** = **`NOT_METROBANK_CLIENT`** and **`metrobank_deposit_repayment_plan_line`** = **`, "metrobank_deposit_repayment_plan": "WILL_OPEN_METROBANK_DEPOSIT"`** (leading comma — appended after the second PEP boolean in **Create** / **eligibility-preview** / edge payloads). That matches a new client who will open a Metrobank deposit for **ADA**; **Step 7c** (**`POST …/metrobank-deposit-account/confirm`**) then returns **200** after **POST …/documents** (instead of **409** from a missing plan). The enum must stay a **quoted** JSON string.
