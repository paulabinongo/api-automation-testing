...
| **§5**    | **Business view**: happy-path table (**§5.0**), **Metrobank Home Loan** E2E (**§5.0.1**), routes cheat sheet, product fields (**Personal** + **HOME_LOAN**), **PEP** gate, edge cases, **real bank vs mock**. |
...
| **Developer changing routes or payloads** | [Modify the project](#4-modify-the-project), [Update Swagger / OpenAPI](#34-update-swagger-and-openapi); then use the **checklist table** under [§5.0](#50-step-by-step-happy-path) (_When your real product differs_).                               |
...
| **OpenAPI JSON** (machine-readable) | [http://127.0.0.1:8765/openapi.json](http://127.0.0.1:8765/openapi.json) — same contract Swagger renders; **`info.version`** (e.g. **0.8.17**) tracks releases; describes **PERSONAL_LOAN** and **HOME_LOAN** (see [§5.0.1](#501-metrobank-home-loan--full-e2e-happy-path)); **`info.description`** points at **`loan-products/**` modules (§**6**) |
...
- **`GET /v1/reference/loan-products`** — catalogue for all registered **`product_code`** values (**PERSONAL_LOAN**, **HOME_LOAN**, …): terms, limits, LOVs, fees — built from **`javascript/lib/loanProductCatalog.js`** (**`buildLoanProductReferencePayload`**).
- **`GET /v1/reference/loan-computation-preview`** — preview of interest, fees, net proceeds, and **EIR** for sample **`principal_cents`** / **`term_months`**; optional **`product_code`** / **`loan_purpose`** (**HOME_LOAN**). Dispatch: **`javascript/lib/loan-products/computationRegistry.js`**.
...
| **1c-MB** _(conditional)_     | Metrobank **deposit for ADA** confirmation (**Step 7c**).       | `POST /v1/loan-applications/{applicationId}/metrobank-deposit-account/confirm`                                             | **Personal Loan** only. Body `{}`. When **`metrobank_client_type`** is **`NOT_METROBANK_CLIENT`** or **`EXISTING_CLIENT_CREDIT_CARD`** and **`additional_information.metrobank_deposit_repayment_plan`** is **`WILL_OPEN_METROBANK_DEPOSIT`**. **400** if **`EXISTING_CLIENT_DEPOSIT_ACCOUNT`** or **HOME_LOAN** (not applicable). **409** if **1c** not done, plan is missing or not **`WILL_OPEN_METROBANK_DEPOSIT`**, or status is past **underwriting** queue (not **DRAFT** / **SUBMITTED** / **IN_PROCESSING** / **CREDIT_COMPLETED** / **IN_UNDERWRITING**). Re-runs **eligibility** (**422** if failed). Sets **`metrobank_deposit_account_confirmed_at`** (cleared when **PATCH** changes **`additional_information`** or **`metrobank_client_type`**). | Bearer + **application_id** after **1c** (up to **IN_UNDERWRITING**)     | Timestamp required before **underwriting** can **APPROVE** / **CONDITIONAL** for applicable **Personal Loan** paths (not before **submit**). |
...
| **2**                         | Send the application to processing.                             | `POST /v1/loan-applications/{applicationId}/submit`                                                                        | No body. **1c** required (**409** if skipped). **Personal Loan:** **1c-MB** is **not** required for **submit**; **underwriting** **APPROVE** / **CONDITIONAL** stay **422** until **`metrobank_deposit_account_confirmed_at`** when **`NOT_METROBANK_CLIENT`** or **`EXISTING_CLIENT_CREDIT_CARD`**. **HOME_LOAN:** no **`metrobank_deposit_account_confirmed_at`** requirement. If **either** PEP boolean **`true`**, **1c-PEP** required (**409** if skipped).                                                                                                                                                                                                                                                                                                 | **application_id** from step 1                                           | Status **SUBMITTED**.                                                                                                                        |
...
This is the same **API sequence** as [§5.0](#50-step-by-step-happy-path), told for **Metrobank Home Loan (Residential)** (`product_code`: **`HOME_LOAN`**). Intake omits **`metrobank_client_type`** (ADA deposit confirm applies to **Personal Loan** only in this sandbox). **Step 7** still requires **`POST …/documents`**; **Step 6** PEP rules and **1c-PEP** are unchanged.
...
| Term                              | **240** months (**20** years; within **25**-year resident cap for house-and-lot in **`homeLoanCatalog.js`**)                                 |
...
**Optional before submit:** **`POST /v1/loan-applications/eligibility-preview`** or **`GET /v1/reference/loan-computation-preview`** with **`product_code=HOME_LOAN`**, **`loan_purpose`**, **`principal_cents`**, **`term_months`** — pricing uses level annual % buckets (1–5 year lock-in); Home Equity purposes use the +**1%** tier (see catalogue).
...
Minimum age **21**; gross monthly family income **≥ PHP 40,000**; employed **≥ 2** years with current employer (or **≥ 3** years in business if self-employed); good credit / **no adverse findings**; collateral **residential**. See **`homeLoanEligibility.js`** and **`HOME_LOAN_PRODUCT.eligibility`** in **`javascript/lib/homeLoanCatalog.js`**.
...
3. **`POST /v1/loan-applications`** — **DRAFT** with **`HOME_LOAN`** body (no **`metrobank_client_type`**): **`loan_purpose`**, **`principal_cents`**, **`term_months`**, **`additional_information`** (PEP booleans, appraisal, applicant category, collateral flags, **`no_adverse_credit_history`**), **`borrower`**, **`employment`**.
4. _(Optional)_ **`POST /v1/loan-applications/eligibility-preview`** or **`PATCH`**, **`GET …/computation-preview`**.
5. **`POST /v1/loan-applications/{id}/documents`** — **`primary_id_document_type`** must match borrower (e.g. **PASSPORT**).
6. _(Skip **1c-MB** for Home Loan in this sandbox.)_
...
17. _(Optional)_ **`GET /v1/loans/{loanId}/payment-schedule`**.
...
6. (**Personal Loan** only.) If intake is **`NOT_METROBANK_CLIENT`** or **`EXISTING_CLIENT_CREDIT_CARD`** with **`WILL_OPEN_METROBANK_DEPOSIT`**, you may **POST …/metrobank-deposit-account/confirm** after documents (through **IN_UNDERWRITING**, until a decision **APPROVE** / **CONDITIONAL**) to set **`metrobank_deposit_account_confirmed_at`** — required before that decision succeeds, not before **submit**. Skip when the borrower already has a Metrobank **deposit** account (**`EXISTING_CLIENT_DEPOSIT_ACCOUNT`**). **HOME_LOAN** does not use this call (**400** not applicable).
...
| —    | Payment / EIR preview                              | `GET /reference/loan-computation-preview?principal_cents=&term_months=` — **public**; optional **`product_code`** / **`loan_purpose`**; **`javascript/lib/loan-products/computationRegistry.js`** → **`personal-loan/personalLoanComputation.js`** / **`home-loan/homeLoanComputation.js`** |
...
| —    | Metrobank deposit confirm (**Personal Loan** only) | `POST /loan-applications/{applicationId}/metrobank-deposit-account/confirm` — after **1c**, before **underwriting** **APPROVE**, when **`WILL_OPEN_METROBANK_DEPOSIT`**; **HOME_LOAN** → **400**; not a **submit** gate (**§5.0** row **1c-MB**)                                            |
...
| 2    | Submit                                             | `POST .../submit` → **SUBMITTED** (**1c** + conditional **pep-clearance**; **metrobank confirm** gates **Personal Loan** **approval** only, not **submit** or **HOME_LOAN**)                                                                                                                |
...
- **Codes:** **`PERSONAL_LOAN`** (unsecured) and **`HOME_LOAN`** (Metrobank Home Loan — residential mortgage; **[§5.0.1](#501-metrobank-home-loan--full-e2e-happy-path)**).
...
This is **`metrobank_client_prerequisite`** on **`GET /v1/reference/loan-products`**. **Personal Loan** repayments are collected by **automatic debit (ADA)** against a **Metrobank deposit account** only. A Metrobank **credit card alone** (no Metrobank **deposit** account) is **not** sufficient — the borrower must **already have** or **commit to opening** a Metrobank deposit account for **ADA**.
...
**Underwriting** blocks **APPROVE** / **CONDITIONAL** when **`metrobank_client_type`** is **`NOT_METROBANK_CLIENT`** or **`EXISTING_CLIENT_CREDIT_CARD`** and **`metrobank_deposit_account_confirmed_at`** is null (**422**).

**Eligibility** (`javascript/lib/loan-products/personal-loan/personalLoanEligibility.js`): **`metrobank_deposit_for_ada`** documents that a Metrobank deposit for **ADA** is required before **approval**; intake no longer fails solely on plan choice.
...
**Payment preview (calculator):** **`GET /v1/reference/loan-computation-preview`** with query **`principal_cents`** and **`term_months`** returns total interest, monthly amortization, fees, net proceeds, and effective annual **EIR** (%). **PERSONAL_LOAN** uses the add-on model in **`javascript/lib/loan-products/personal-loan/personalLoanComputation.js`**; **HOME_LOAN** uses level annual % buckets in **`javascript/lib/loan-products/home-loan/homeLoanComputation.js`** — both via **`javascript/lib/loan-products/computationRegistry.js`**. **After** the borrower creates an application, **`GET /v1/loan-applications/{applicationId}/computation-preview`** (Bearer required) returns the **same** shape for that draft’s **`product_code`** — response includes **`application_id`**. **`LoanApiClient.getLoanComputationPreview({ … })`** and **`getLoanComputationPreviewForApplication(id)`** wrap the two routes.
...
**Question this section answers:** _How close is this practice API to a real bank?_
...
**Contract parity:** The collection **description** (overview + flow folders) matches **`javascript/mock-server/openapi.json`** (**`info.version`** **0.8.17**: **`info`**, **Tags**, **`POST …/compliance/pep-clearance`**, **`HOME_LOAN`** fields, **`info.description`** source-map for **`loan-products/**`) and [§5 Loan lifecycle](#5-loan-lifecycle-business-view) — [§5.0](#50-step-by-step-happy-path), [§5.0.1](#501-metrobank-home-loan--full-e2e-happy-path), Step **7b**, [§5.4](#54-mapping-to-production-bank-lifecycle). Update **OpenAPI**, **§5**, **§6** (file map), and **Postman** together when routes, PEP rules, product codes, or module paths change.
...
3. **Collection Runner** → **Loan Lifecycle API** → enable **one** folder → **Run** (leave **Edge cases** unchecked):
   ...
   - **Flow — Metrobank Home Loan (happy path)** — **HOME_LOAN** sample (**Passport**, **PURCHASE_HOUSE_AND_LOT**, **240** months); same steps from **Login** through **Payoff**; **1c-MB** expects **400**; step **13** uses **`home_loan_payment_amount_cents`** (keep in sync with **1. Create** **`principal_cents`**). Walkthrough: [§5.0.1](#501-metrobank-home-loan--full-e2e-happy-path).
