...
| **OpenAPI JSON** (machine-readable) | [http://127.0.0.1:8765/openapi.json](http://127.0.0.1:8765/openapi.json) — same contract Swagger renders; **`info.version`** (e.g. **0.8.16**) tracks releases; describes **PERSONAL_LOAN** and **HOME_LOAN** (see [§5.0.1](#501-metrobank-home-loan--full-e2e-happy-path)) |
...
| —    | Payment / EIR preview                              | `GET /reference/loan-computation-preview?principal_cents=&term_months=` — **public**; optional **`product_code`** / **`loan_purpose`** (**`loan-products/personal-loan/personalLoanComputation.js`** vs **`homeLoanComputation.js`**)            |
...
Logic lives in **`javascript/lib/loan-products/personal-loan/personalLoanComputation.js`**.
...
| `javascript/lib/loan-products/personal-loan/personalLoanComputation.js` | Add-on interest, fees, net proceeds, **EIR** — used by **`GET /reference/loan-computation-preview`**           |
...
**Contract parity:** The collection **description** (overview + flow folders) matches **`javascript/mock-server/openapi.json`** (**`info.version`** **0.8.16**: **`info`**, **Tags**, **`POST …/compliance/pep-clearance`**, **`HOME_LOAN`** fields) and [§5 Loan lifecycle](#5-loan-lifecycle-business-view) — [§5.0](#50-step-by-step-happy-path), [§5.0.1](#501-metrobank-home-loan--full-e2e-happy-path), Step **7b**, [§5.4](#54-mapping-to-production-bank-lifecycle). Update **OpenAPI**, **§5**, and **Postman** together when routes, PEP rules, or product codes change.
