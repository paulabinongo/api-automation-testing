...
// Public endpoint to get loans by type (no authentication required)
v1.get('/loans-by-type', (req, res) => {
  const { 
    product_code, 
    loan_type, 
    status, 
    ...
    page = 1,
    limit = 10,
    loan_category // Filter by loan category based on hierarchy
  } = req.query
  
  // Get all loans from the loans Map
  ...
  if (product_code) {
    filteredLoans = filteredLoans.filter(loan => loan.product_code === product_code)
  }
  
  // Loan category filtering based on hierarchy - focus on specific loan types and children
  if (loan_category) {
    const loanCategoryMappings = {
      ...
      "business_or_commercial": ["BUSINESS_LOAN_WORKING_CAPITAL", "BUSINESS_LOAN_EQUIPMENT_FINANCING", "BUSINESS_LOAN_COMMERCIAL_REAL_ESTATE"],
      // Parent categories
      "personal_loan_type": ["PERSONAL_LOAN", "PERSONAL_LOAN_QUICK_CASH", "PERSONAL_LOAN_PREMIUM", "PERSONAL_LOAN_SALARY", "HOME_LOAN", "CAR_LOAN"],
      "business_loan_type": ["BUSINESS_LOAN_WORKING_CAPITAL", "BUSINESS_LOAN_EQUIPMENT_FINANCING", "BUSINESS_LOAN_COMMERCIAL_REAL_ESTATE"],
      // Legacy support
      "all_personal": ["PERSONAL_LOAN", "PERSONAL_LOAN_QUICK_CASH", "PERSONAL_LOAN_PREMIUM", "PERSONAL_LOAN_SALARY", "HOME_LOAN", "CAR_LOAN"],
      "all_business": ["BUSINESS_LOAN_WORKING_CAPITAL", "BUSINESS_LOAN_EQUIPMENT_FINANCING", "BUSINESS_LOAN_COMMERCIAL_REAL_ESTATE"]
    }
    
    const targetCodes = loanCategoryMappings[loan_category]
    if (targetCodes) {
      filteredLoans = filteredLoans.filter(loan => targetCodes.includes(loan.product_code))
    }
  }
  
  if (loan_type) {
    filteredLoans = filteredLoans.filter(loan => loan.loan_type === loan_type)
  }
  ...
  if (min_amount) {
    filteredLoans = filteredLoans.filter(loan => loan.principal_cents >= parseInt(min_amount))
  }
  
  if (max_amount) {
    filteredLoans = filteredLoans.filter(loan => loan.principal_cents <= parseInt(max_amount))
  }
  
  if (created_after) {
    const dateFilter = new Date(created_after)
    filteredLoans = filteredLoans.filter(loan => new Date(loan.created_at) >= dateFilter)
  }
  ...
  if (search) {
    ...
    filteredLoans = filteredLoans.filter(loan => 
      ...
      loan.product_code.toLowerCase().includes(searchTerm) ||
  ...
  let sortedLoans = filteredLoans.sort((a, b) => {
    let aVal = a[sort_by] || ''
    let bVal = b[sort_by] || ''
    
    if (sort_by === 'principal_cents' || sort_by === 'created_at') {
  ...
  const paginatedLoans = sortedLoans.slice(offset, offset + limitNum)
  
  // Enrich loans with additional data (like Metrobank Home Loan lifecycle phase)
  const enrichedLoans = paginatedLoans.map(loan => enrichLoanOut(loan))
  ...
  res.json({
    ...
    pagination: {
      page: pageNum,
      limit: limitNum,
      total_count: filteredLoans.length,
      total_pages: Math.ceil(filteredLoans.length / limitNum)
    },
    filters: {
      product_code: product_code || null,
      loan_type: loan_type || null,
      loan_category: loan_category || null,
      status: status || null,
    ...
    note: 'Public endpoint - no authentication required. Returns loans filtered by type with hierarchy: Personal (Personal, Home, Car) and Business (Business/Corporate).'
  })
})

v1.get('/v1/loans-by-type', (req, res) => {
  const { 
    product_code, 
    loan_type, 
    status, 
...
// NEW: GET endpoint for loan types hierarchy
v1.get('/loan-types-hierarchy', async (req, res) => {
  // Import the loan products catalog to get the product information
  const { LOAN_PRODUCTS_BY_CODE } = await import('../src/loanProductCatalog.js')
  const { PRODUCT_LOAN_TYPE } = await import('../src/utils/productLoanTaxonomy.js')
  
  // Build the hierarchical structure with strict parent-child relationships
  const loanTypesHierarchy = {
    personal: {
      name: "Personal Loan Type",
      description: "Consumer and retail loan products for individuals",
      children: [
        {
          name: "Personal",
...
v1.get('/v1/loan-types-hierarchy', async (req, res) => {
  // Import the loan products catalog to get the product information
  const { LOAN_PRODUCTS_BY_CODE } = await import('../src/loanProductCatalog.js')
  const { PRODUCT_LOAN_TYPE } = await import('../src/utils/productLoanTaxonomy.js')
...
// Add non-v1 routes for compatibility
app.get('/loan-types-hierarchy', async (req, res) => {
  // Import the loan products catalog to get the product information
  const { LOAN_PRODUCTS_BY_CODE } = await import('../src/loanProductCatalog.js')
  const { PRODUCT_LOAN_TYPE } = await import('../src/utils/productLoanTaxonomy.js')
