...
export function buildLoanProductReferencePayload() {
  return {
    ...
    note: '**All amounts** in **PHP centavos** on **principal_cents** and **employment.gross_monthly_income_cents** (×12 vs **min_annual_income_cents** on the product). Principal: **PHP 20,000**–**2,000,000** (whole pesos only). **loan_purpose**, **term_months** (**12|18|24|36**), **additional_information** (PEP), **borrower** (Present Home Address + **philippine_address_sample_rows**, optional **home_phone** / **landline_area_code_options**, names, consents, Step 3 ID subset on create), **employment** (**employer_address** when **EMPLOYED**; optional **business_mobile_phone**, **business_phone**). Step 7 **POST …/documents** must match declared ID. **GET /v1/reference/loan-computation-preview** (add-on, EIR, amortization, fees, net proceeds).',
  }
}
