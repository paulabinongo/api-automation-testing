...
| Concern | File(s) |
|--------|---------|
| Catalogue (purposes, LTV/term rules, IDs, rates, fees text) | **`javascript/lib/homeLoanCatalog.js`** |
| Intake validation | **`validateHomeLoanIntakeShape`** in **`javascript/lib/loanProductCatalog.js`** |
| Eligibility | **`javascript/lib/homeLoanEligibility.js`** (registered in **`../registry.js`**) |
| Payment preview (level annual % buckets) | **`javascript/lib/homeLoanComputation.js`** (**`../computationRegistry.js`**) |
